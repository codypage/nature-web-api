...
const createSendToken = (user, statusCode, req, res) => {
  ...
  // Remove password from the console output.
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError('You are not loggin in, please login and try again', 401)
    );
  }
  // 2) Verify the token
  const decoded = await promisify(jwt.verify)(token, process.env.JWT_SECRET);

  // 3) If the verification was successful, check if the user still exists
  const currentUser = await User.findById(decoded.id);
  if (!currentUser) {
    return next(
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) Set new password if the token hasn't expired, and the user exists.
  if (!user) {
